...
![Mosaic Lab 界面：图片队列、画布、打码区域与导出前自检面板](docs/img/hero.png)
...
![原图与打码后对比：三条文字行用实心覆盖，头像用马赛克 24px](docs/img/before-after.png)
...
同一段证件号在四种处理下的真实导出对比（不是示意图，全部来自工具实际输出）：

![同一区域四种强度对比：原始 / 实心 / 马赛克 24px / 马赛克 12px + 块内加噪](docs/img/strength-compare.png)
...
![移动端界面](docs/img/mobile.png)

每次导出的自检结果就地显示在右下角：

![导出后自检：逐区域破坏度 PSNR 与无 EXIF / GPS / 内嵌缩略图核对](docs/img/selfcheck.png)
